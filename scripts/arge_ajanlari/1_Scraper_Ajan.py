...
# .env dosyasını bu script'in bulunduğu klasörden oku
SCRIPT_DIR = Path(__file__).resolve().parent
load_dotenv(SCRIPT_DIR / ".env")

# SUPABASE BAĞLANTISI (.env dosyasından güvenli şekilde okunuyor)
SUPABASE_URL = os.environ.get("SUPABASE_URL")
SUPABASE_KEY = os.environ.get("SUPABASE_SERVICE_KEY")

if not SUPABASE_URL or not SUPABASE_KEY:
    print("🔴 [KRİTİK HATA] .env dosyasında SUPABASE_URL veya SUPABASE_SERVICE_KEY bulunamadı!")
    print(f"   Aranan .env konumu: {SCRIPT_DIR / '.env'}")
    exit(1)
